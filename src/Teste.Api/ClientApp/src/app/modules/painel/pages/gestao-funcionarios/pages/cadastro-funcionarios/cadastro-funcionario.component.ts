...
export class CadastrarFuncionarioComponent extends BaseFormComponent implements OnInit {
  ...
  submit(): void {
    ...
    if (this.form.value.habilidades) {
      gpids = this.form.value.habilidades.map((obj) => {
        return obj.id;
      });
    }
    ...
    this.spinner.show();
    this.funcionariosService.salvar(data)
